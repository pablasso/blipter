...
$(document).ready(function(){
	...
	var socket = new Pusher('4136df60be0891707b27');
	...
	presenceChannel.bind('pusher:subscription_succeeded', function(member_list){
	  $('#members').empty();
		...
		for (var member in member_list._members_map) {
			var p = $('<p>', { html: "<span>UserID: </span>" + member, id: 'member_' + member, "class": 'member', "style": 'display:none;' } );
			$('#members').append(p);
			$('#member_' + member).show();
		};
	});
